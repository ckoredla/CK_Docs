export const historicalExpansions: Record<string, { context: string[]; validation: string[] }> = {
  '2026-03-ai-agent-assurance': {
    ...
    validation: [
      'Evaluation should inspect the trajectory, not merely the final prose. Test tool selection, parameter construction, source eligibility, recovery from partial failure, repeated execution, malicious or misleading retrieved content, stale state, and attempts to exceed scope. Complete traces should allow an assessor to reproduce every observation and proposed action.',
