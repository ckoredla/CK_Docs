...
export function IssueArticle({ slug }: { slug: string }) {
  ...
  return <ArticleShell article={article}>
    ...
    {content.sections.map((section, index) => <section key={section.title}><h2>{index + 1}. {section.title}</h2>{section.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</section>)}
