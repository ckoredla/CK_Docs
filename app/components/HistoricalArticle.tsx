...
export function HistoricalArticle({slug}:{slug:string}){
  ...
  return <ArticleShell article={article}>
    ...
    <ReferenceArchitecture id={`${slug}-context`} title={`Decision architecture for ${issue.domain}`} caption="System boundaries, evidence custody, authority, and feedback are shown as separate architectural concerns so reviewers can inspect where meaning changes and where control resides." domain={issue.domain} tags={issue.tags} ata={issue.ata}/>
